class SalesCartManager {
    ...
    addToCart() {
        if (!this.selectedVariant) { alert("Please select a variant"); return; }
    ...
    render() {
        ...
        this.cart.forEach(item => {
            ...
            tr.innerHTML = `<td class="p-2 font-bold text-brand-700"><div>` + item.pName + `</div><div class="text-[11px] text-brand-300">` + item.vName + `</div></td><td class="p-2 font-semibold text-center">` + item.qty + `</td><td class="p-2 text-brand-300">₱` + item.price.toFixed(2) + `/` + ul + pc + `</td><td class="p-2 font-bold text-brand-500">₱` + item.subtotal.toFixed(2) + `</td><td class="p-2 text-right"><button type="button" onclick="cartManager.removeFromCart(` + item.vid + `)" class="w-6 h-6 rounded-full bg-red-50 text-red-500 font-bold text-xs">&times;</button></td>`;
    ...
    submitSale(e) {
        ...
        const total = this.cart.reduce((s, i) => s + i.subtotal, 0);
        const paid = parseFloat(this.paidInput?.value || 0) || 0;
        if (paid < total) { e.preventDefault(); alert("Insufficient payment. Total: ₱" + total.toFixed(2)); return; }
