...
try {
    ...
    foreach ($cartItems as $item) {
        ...
        // Use SaleLineItemCalculator for unified pricing logic
    ...
    // 3. Payment Validation using SaleLineItemCalculator
    if (!SaleLineItemCalculator::validatePayment($amountPaidInput, $totalAmount)) {
        $_SESSION['flash_error'] = 'Amount paid (₱' . number_format($amountPaidInput, 2) . ') is insufficient. Total is ₱' . number_format($totalAmount, 2) . '.';
        header('Location: /staff/new-sale.php');
        exit;
    }

    $changeAmount = SaleLineItemCalculator::calculateChange($amountPaidInput, $totalAmount);
