    ...
    <div class="bg-gradient-to-r from-brand-700 via-brand-600 to-brand-500 text-white p-6 sm:p-8 rounded-md shadow-card flex flex-col sm:flex-row sm:items-center justify-between gap-5">
        <div>
            <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 border border-white/20 text-white text-xs font-semibold mb-2">
                <span class="w-2 h-2 rounded-full bg-green-400 animate-pulse"></span> Terminal Ready
            </div>
        ...
        <div class="w-full rounded-md border border-brand-200 bg-white shadow-card">
            ...
            <div class="w-full p-6">
                ...
                <div class="flex items-center">
                    <span class="items-center font-semibold inline-flex justify-center px-2 py-1 rounded-md bg-green-50 text-green-600 mr-2 text-sm">
                ...
                <table class="w-full text-left text-sm min-w-[550px]">
                    <thead>
                        <tr class="border-b border-brand-200">
                            <th class="px-4 py-3 text-sm font-semibold text-brand-300">Transaction #</th>
                            <th class="px-4 py-3 text-sm font-semibold text-brand-300">Total Amount</th>
                            <th class="px-4 py-3 text-sm font-semibold text-brand-300">Amount Paid</th>
                            <th class="px-4 py-3 text-sm font-semibold text-brand-300">Change</th>
                            <th class="px-4 py-3 text-sm font-semibold text-brand-300">Time</th>
                            <th class="px-4 py-3 text-sm font-semibold text-brand-300 text-right">Receipt</th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <?php else: ?>
                            <?php foreach ($myRecentSales as $s): ?>
                                <tr class="border-b border-brand-100 hover:bg-brand-50 transition-colors">
                                    <td class="px-4 py-3 font-semibold text-brand-700 font-mono text-sm"><?= e($s['transaction_number']); ?></td>
                                    <td class="px-4 py-3 font-bold text-brand-500 text-sm">₱<?= number_format($s['total_amount'], 2); ?></td>
                                    <td class="px-4 py-3 text-brand-700 text-sm">₱<?= number_format($s['amount_paid'], 2); ?></td>
                                    <td class="px-4 py-3 text-brand-300 text-sm">₱<?= number_format($s['change_amount'], 2); ?></td>
                                    <td class="px-4 py-3 text-sm text-brand-300"><?= date('h:i A', strtotime($s['created_at'])); ?></td>
