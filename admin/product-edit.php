...
<div class="max-w-4xl mx-auto py-6 space-y-6">
    <div class="w-full rounded-md border border-brand-200 bg-white shadow-card p-6 flex items-center justify-between">
        <h1 class="text-xl font-bold text-brand-700">Edit: <?= e($product['name']); ?></h1>
    ...
    <form action="/actions/update-product.php" method="POST" enctype="multipart/form-data" class="w-full rounded-md border border-brand-200 bg-white shadow-card p-6 space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="md:col-span-2 space-y-4">
                <div>
                    ...
                    <input type="text" name="name" value="<?= e($product['name']); ?>" required class="w-full px-4 py-2.5 rounded-md border border-brand-200 text-sm text-brand-700">
                </div>
            </div>

            <div class="bg-brand-50 p-4 rounded-md border border-brand-200 text-center">
                <label class="block text-sm font-semibold text-brand-700 mb-3">Photo</label>
                <img id="photoPreview" src="<?= e(getProductImage($product['image'], $product['name'])); ?>" alt="" class="w-28 h-28 object-cover rounded mx-auto mb-3">
                <label for="image" class="inline-block px-3 py-1.5 bg-white border border-brand-200 text-brand-700 text-xs font-semibold rounded cursor-pointer">Change</label>
        ...
        <div class="pt-4 border-t border-brand-200 flex justify-end gap-3">
            <a href="/admin/products.php" class="px-5 py-2.5 rounded-md border border-brand-200 text-brand-700 font-semibold text-sm">Cancel</a>
