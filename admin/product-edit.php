...
<div class="max-w-4xl mx-auto py-6 space-y-6">
    ...
    <form action="/actions/update-product.php" method="POST" enctype="multipart/form-data" class="w-full rounded-md border border-brand-200 bg-white shadow-card p-4 sm:p-6 space-y-6">
            ...
            <div class="space-y-3">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-12 gap-3 items-end bg-brand-50 p-4 rounded-md border border-brand-200">
                    ...
                    <div class="sm:col-span-2 lg:col-span-3">
                        ...
                        <input type="text" name="variant_name[]" value="<?= e($v['variant_name']); ?>" class="w-full px-3 py-2 rounded-md border border-brand-200 text-sm text-brand-700 focus:outline-none focus:border-brand-500">
                    </div>
                    
                    <!-- Selling Unit -->
                    <div class="sm:col-span-2 lg:col-span-2">
                        <label class="block text-xs font-semibold text-brand-700 mb-1.5">Selling Unit</label>
                        <select name="selling_unit[]" class="w-full px-3 py-2 rounded-md border border-brand-200 text-sm text-brand-700 focus:outline-none focus:border-brand-500">
                    ...
                    <div class="sm:col-span-1 lg:col-span-2">
                        ...
                        <input type="number" name="pieces_per_unit[]" value="<?= intval($v['pieces_per_unit'] ?? 1); ?>" min="1" class="w-full px-3 py-2 rounded-md border border-brand-200 text-sm text-brand-700 focus:outline-none focus:border-brand-500">
                    </div>
                    
                    <!-- Price -->
                    <div class="sm:col-span-1 lg:col-span-2">
                        <label class="block text-xs font-semibold text-brand-700 mb-1.5">Price (₱)</label>
                        <input type="number" name="variant_price[]" value="<?= number_format($v['price'], 2); ?>" step="0.01" class="w-full px-3 py-2 rounded-md border border-brand-200 text-sm text-brand-700 focus:outline-none focus:border-brand-500">
                    </div>
            ...
            <div id="newVariantsContainer" class="space-y-3">
                <div class="grid grid-cols-1 sm:grid-cols-12 gap-3 items-center new-variant-row bg-brand-50 p-3 rounded-md border border-brand-200">
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <input type="number" min="1" step="1" name="new_pieces_per_unit[]" value="1" placeholder="Qty" class="w-full px-3 py-2 rounded-md border border-brand-200 text-sm text-brand-700 focus:outline-none focus:border-brand-500">
                    </div>
                    <div class="sm:col-span-2">
                    ...
                    <div class="sm:col-span-2 flex items-end">
                        <button type="button" class="removeNewVariantBtn w-full px-3 py-2 bg-brand-100 hover:bg-brand-200 text-brand-500 font-semibold text-sm rounded-md border border-brand-200 transition-colors focus:outline-none">
                            Remove
                        </button>
                    </div>
                </div>
        ...
        <div class="pt-4 border-t border-brand-200 flex flex-col sm:flex-row sm:justify-between gap-3">
            <div>
                <button type="button" id="deleteProductBtn" class="px-4 sm:px-5 py-2.5 rounded-md bg-brand-1000 hover:bg-brand-600 text-white font-semibold text-sm transition-colors">
                    🗑️ Delete Product
                </button>
            </div>
...
<div id="deleteSuccessModal" class="hidden fixed inset-0 bg-brand-900/60 flex items-center justify-center z-50 p-4">
    <div class="bg-white rounded-lg shadow-lg max-w-md w-full p-6 space-y-4">
        ...
        <div class="flex gap-3 pt-2">
            <button type="button" id="okDeleteBtn" class="flex-1 px-4 py-2.5 rounded-md bg-brand-1000 hover:bg-brand-600 text-white font-semibold text-sm transition-colors">
                OK
            </button>
        </div>
    </div>
...
<div id="deleteModal" class="hidden fixed inset-0 bg-brand-900/60 flex items-center justify-center z-50 p-4">
    <div class="bg-white rounded-lg shadow-lg max-w-md w-full p-6 space-y-4">
        ...
        <div class="flex gap-3 pt-2">
            ...
            <button type="button" id="confirmDeleteBtn" class="flex-1 px-4 py-2.5 rounded-md bg-brand-1000 hover:bg-brand-600 text-white font-semibold text-sm transition-colors">
                Delete Product
            </button>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    removeVariantBtns.forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            const variantNameInput = variantRow.querySelector('input[name*="variant_name"]');
...
// ===== ADD NEW VARIANTS FUNCTIONALITY =====
document.addEventListener('DOMContentLoaded', () => {
    ...
    function createNewVariantRow() {
        ...
        row.className = 'grid grid-cols-1 sm:grid-cols-12 gap-3 items-center new-variant-row bg-brand-50 p-3 rounded-md border border-brand-200';
        row.innerHTML = `
            <div class="sm:col-span-3">
                <label class="block text-xs font-semibold text-brand-700 mb-1">Variant Name</label>
                <input type="text" name="new_variant_name[]" placeholder="Variant Name (e.g. Small Tray)" class="w-full px-3 py-2 rounded-md border border-brand-200 text-sm text-brand-700 focus:outline-none focus:border-brand-500">
        ...
        row.querySelector('.removeNewVariantBtn').addEventListener('click', (e) => {
            e.preventDefault();
            row.remove();
        });
    ...
    addNewVariantBtn?.addEventListener('click', (e) => {
        e.preventDefault();
        const newRow = createNewVariantRow();
        newVariantsContainer.appendChild(newRow);
    });

    document.addEventListener('click', (e) => {
        if (e.target.classList.contains('removeNewVariantBtn')) {
            e.preventDefault();
